...
class App(design.Ui_Form, QMainWindow):
    ...
    def resultF1aHandler(self):
        if self.boxF1a.text() == '':
            ...
            return
        self.boxF1aAns.setText(' '.join(map(str, f1a(boxRipper(self.boxF1a)))))

    def resultF1bHandler(self):
        if self.boxF1b.text() == '':
            QMessageBox.about(self, 'Ошибка', "Второе поле не может быть пустым!")
            return
        self.boxF1bAns.setText(' '.join(map(str, f1b(boxRipper(self.boxF1b)))))
